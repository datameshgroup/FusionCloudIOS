...
//    }
    ...
    @IBAction func btnSettingsTapped(_ sender: UIImageView){
        let storyboard = UIStoryboard(name: "Main", bundle: nil)
        let vc = storyboard.instantiateViewController(withIdentifier: "SettingsStoryboard")
    ...
    func doTransactionStatus() {
        ...
        if (secondsRemaining > 0)
        {
            ...
            fusionClient.messageHeader?.messageCategory = MessageCategory.TransactionStatus
...
//            loginRequest.operatorID = "sfsuper"
    ...
    func doAbort(abortReason: String) {
        fusionClient.messageHeader?.messageCategory = MessageCategory.Abort
    ...
    func doPayment(paymentType: String, requestedAmount: NSDecimalNumber, tipAmount: NSDecimalNumber){
        ...
        fusionClient.messageHeader?.messageCategory = MessageCategory.Payment
        ...
        let paymentData = PaymentData(paymentType: PaymentType(rawValue: paymentType) ?? PaymentType.Normal) // paymentType = Normal|Refund
    ...
    func paymentResponseReceived(client: FusionClient, messageHeader: MessageHeader, paymentResponse: PaymentResponse) {
        ...
        let entryMode = cardData?.entryMode ?? EntryMode.Unknown
        let paymentInstrumentType = paymentResult?.paymentInstrumentData?.paymentInstrumentType ?? PaymentInstrumentType.Unspecified
        ...
        else if(paymentResponse.response?.errorCondition==ErrorCondition.Cancel){
    ...
    func transactionStatusResponseReceived(client: FusionClient, messageHeader: MessageHeader, transactionStatusResponse: TransactionStatusResponse) {
        ...
        let paymentBrand = cardData?.getPaymentBrand()
        let entryMode = cardData?.entryMode ?? EntryMode.Unknown
        let paymentInstrumentType = pResult?.paymentInstrumentData?.paymentInstrumentType ?? PaymentInstrumentType.Unspecified
        ...
        if(success && errorCondition==nil) {
            ...
            if(errorCondition==ErrorCondition.InProgress && secondsRemaining>0){
                showReceipt(doShow: false)
                DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(10)) {
                    self.doTransactionStatus()
                }
            } else if(errorCondition==ErrorCondition.Cancel){
                showReceipt(doShow: true)
                txtPaymentResult.backgroundColor = UIColor.systemYellow;
                txtPaymentResult.textColor = UIColor.white;
                txtPaymentResult.text = "PAYMENT CANCELLED"
